(function(root, factory) {
  ...
  Compiler.prototype.processfn = function processfn(v) {
    ...
    var blockOutput = this.processblock(block[1]);

    var defs = this.symbolTable.popScope();
    for (var prop in defs) {
      output += 'var ' + prop + ' = ' + defs[prop] + ';\n';
    }
  ...
  Compiler.prototype.processdef = function processcall(params) {
  ...
  function compile(templateName, src) {
    ...
    try {
      ...
      var defs = codeGenerator.symbolTable.popScope();
      var defStr = '';
      for (var prop in defs) {
